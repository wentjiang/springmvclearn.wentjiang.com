...
/**
 * Created by jiangwentao on 2/27/2017 3:20 PM.
 */
public class DateTimeEditor extends PropertyEditorSupport {
    ...
    public DateTimeEditor(List<SimpleDateFormat> dateFormat, boolean allowEmpty, int exactDateLength) {
        this.dateFormat = dateFormat;
        this.allowEmpty = allowEmpty;
    ...
    public void setAsText(String text) throws IllegalArgumentException {
        if (this.allowEmpty && !StringUtils.isEmpty(text)) {
            ...
            while (i < this.dateFormat.size()) {
                try {
                    this.setValue((this.dateFormat.get(i)).parse(text));
                    break;
